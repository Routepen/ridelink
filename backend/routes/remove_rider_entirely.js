		...
		Route.find(data, function(err, routes) {
			for(let k = 0; k < routes.length; k++){
				...
				if(routes[k].driver.toString() == req.body.userId.toString()){
					...
					for (let i = 0; i < routes[k].confirmedRiders.length; i++) {
						let rider = routes[k].confirmedRiders[i];
					...
					for (let i = 0; i < routes[k].riders.length; i++) {
						let rider = routes[k].riders[i];
						if (rider.toString() == removingId) {
