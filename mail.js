...
function sendMail(options) {
  ...
  if (options.notifyRider) {
    if (options.notifyRider.confirmed) {
      ...
      if (options.route.requireInitialDeposit) {
        ...
        "<span>Awesome! Congrats, " + driversName + " wants to take you on a ride " + date + " at " + time + ". Message them for pickup location details." + initialDepositHTML + "</span><br/><br/>"+
        "<span>Also don't worry, we're not going to take your money and run :) You'll get a full refund if the driver doesn't actually show up. After the ride is over, we'll send you an email asking if " + driversName + " actually showed up.</span><br/><br/>"+
        "<span>It's important to cancel " + cancelLink + " if you have decided not to go with them! It's only fair to the poor souls on the waitlist :)</span><br/><br/>"+
        "<span>Best,</span><br/>"+
    ...
    if (options.notifyRider.paid) {
      ...
      html = "<span>Hi " + recipientName + ",</span><br/><br/>"+
      "<span>Congrats on fully confirming the ride with " + driversName + ". We'll shoot you a final email the day of the ride and if the driver didn't actually show up, you'll have the option to let us know and get a full refund!</span><br/><br/>" +
      "<span>Best</span>,<br/>"+
      "<span>Routepen Team</span>";
    }
    ...
    if (options.notifyRider.offWaitlist) {
      if (options.route.requireInitialDeposit) {
        ...
        html = "<span>Hi " + recipientName + ",</span><br/><br/>"+
        "<span>Congrats on fully confirming the ride with " + driversName + ". Hope you enjoy your ride</span><br/><br/>" +
        "Click " + fbLink + " to message them if you have any further questions.\n\n"+
        "<span>Best</span><br/>"+
        "<span>Routepen Team</span>";
      }
    }
    ...
    if (options.notifyRider.signedUp) {
      ...
      html = "<span>Hi " + recipientName + ",</span><br/><br/>" +
        "<span>Welcome to Routepen! HTML smirk with sunglasses emoji</span><br/><br/>" +
  ...
  if (options.notifyDriver) {
    ...
    if (options.notifyDriver.riderPaid) {
      ...
      html = "<span>Hi " + driversName + ",</span><br/><br/>" +
      "<span>We have some awesome news. " + riderName + " has paid his initial deposit and they're locked in for the ride. On the day of the ride, we'll send you an email confirming that you have taken them and you'll get your money then!</span><br/><br/>" +
      "<span>If interested, visit " + riderName + "'s Facebook profile on your map " + fbLink + " and send them a quick message with details of the ride.</span><br/><br/>" +
      "<span>Best,</span><br/>" +
      "<span>Routepen Team</span>";
    }
    ...
    if (options.notifyDriver.routeCreated) {
      ...
      "Then just sit back and wait for riders to enter their dropoff locations on the map and confirm the ones you like. We'll even help you by giving you an email every time a rider signs up. (Excess riders will automatically be added to the waitlist) :) Hope that helps!\n\n"+
      "Best, \n" +
      "Routepen Team";
      /*
      Old text
